...
<form action="${pageContext.request.contextPath }/customer/find" method="post">
    ...
    <td vAlign=top width="100%" bgColor=#ffffff>
        <table cellSpacing=0 cellPadding=5 width="100%" border=0>
            <tr>
                <td class=manageHead>当前位置：客户管理 &gt; 分页客户列表</td>
            </tr>
            <tr>
                <td height=2></td>
            </tr>
            ...
            <tr>
                <td><span>
                    <div style="line-height: 20px; height: 20px; text-align: right">
                        共[<b>${page.totalElements}</b>]条记录,[<b>${page.totalPages}</b>]页,当前第[<b>${page.number+1}</b>]页
                        <c:if test="${page.number!=0}">
                            [<a href="${pageContext.request.contextPath}/customer/listPage?page=${page.number-1}">前一页</a>]
                        </c:if>
                        <c:if test="${page.number!=page.totalPages-1}">
                            [<a href="${pageContext.request.contextPath}/customer/listPage?page=${page.number+1}">后一页</a>]
                        </c:if>
                    </div>
                </span></td>
            </tr>
